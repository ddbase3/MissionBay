...
<div id="<?php echo htmlspecialchars((string)$this->_['instanceId'], ENT_QUOTES); ?>" class="search-config-admin">
	<h3><?php echo $mbTextEsc('search_services', 'Search Services'); ?></h3>
	...
	<div class="searchcfg-meta">
		...
		<div data-role="loading" class="searchcfg-loading"><?php echo $mbTextEsc('please_wait', 'Please wait...'); ?></div>
	</div>
	...
	<div class="searchcfg-layout">
		<div class="searchcfg-listbox">
			<div class="searchcfg-toolbar">
				<button type="button" data-role="new"><?php echo $mbTextEsc('new_search_service', 'New search service'); ?></button>
			...
			<table class="searchcfg-table">
				<thead>
					<tr>
						...
						<th><?php echo $mbTextEsc('context', 'Context'); ?></th>
						<th><?php echo $mbTextEsc('web', 'Web'); ?></th>
		...
		<div class="searchcfg-formbox">
			<form data-role="form">
				<h4 data-role="legend"><?php echo $mbTextEsc('create_search_service', 'Create search service'); ?></h4>

				<div class="searchcfg-hint" data-role="idhint">
					<?php echo $mbTextEsc('technical_search_service_id_agent_resources_use_this_id_to_resolve_the_configured_search_service', 'Technical search service id. Agent resources use this id to resolve the configured search service.'); ?>
				</div>

				<div class="searchcfg-grid">
					<div class="searchcfg-field">
						<label for="<?php echo htmlspecialchars((string)$this->_['instanceId'], ENT_QUOTES); ?>-id"><?php echo $mbTextEsc('search_service_id', 'Search service id'); ?></label>
					...
					<div class="searchcfg-field">
						<label for="<?php echo htmlspecialchars((string)$this->_['instanceId'], ENT_QUOTES); ?>-model">
							<?php echo $mbTextEsc('model', 'Model'); ?> <span data-role="modelrequiredlabel" style="font-weight:normal;color:#777"><?php echo $mbTextEsc('optional', '(optional)'); ?></span>
						</label>
						<input type="text" id="<?php echo htmlspecialchars((string)$this->_['instanceId'], ENT_QUOTES); ?>-model" name="model" placeholder="<?php echo $mbTextEsc('optional', 'optional'); ?>" autocomplete="off">
					...
					<div class="searchcfg-field searchcfg-field-row">
						<div>
							<label for="<?php echo htmlspecialchars((string)$this->_['instanceId'], ENT_QUOTES); ?>-context"><?php echo $mbTextEsc('search_context_size', 'Search context size'); ?></label>
							<select id="<?php echo htmlspecialchars((string)$this->_['instanceId'], ENT_QUOTES); ?>-context" name="searchContextSize">
								<option value=""><?php echo $mbTextEsc('default', 'default'); ?></option>
								<option value="low"><?php echo $mbTextEsc('low', 'low'); ?></option>
								<option value="medium"><?php echo $mbTextEsc('medium', 'medium'); ?></option>
								<option value="high"><?php echo $mbTextEsc('high', 'high'); ?></option>
							</select>
						</div>

						<div>
							<label for="<?php echo htmlspecialchars((string)$this->_['instanceId'], ENT_QUOTES); ?>-toolchoice"><?php echo $mbTextEsc('tool_choice', 'Tool choice'); ?></label>
							<select id="<?php echo htmlspecialchars((string)$this->_['instanceId'], ENT_QUOTES); ?>-toolchoice" name="toolChoice">
								<option value=""><?php echo $mbTextEsc('default', 'default'); ?></option>
								<option value="auto"><?php echo $mbTextEsc('auto', 'auto'); ?></option>
								<option value="required"><?php echo $mbTextEsc('required', 'required'); ?></option>
							</select>
						</div>
					</div>
					...
					<div class="searchcfg-field searchcfg-field-row">
						<div>
							<label for="<?php echo htmlspecialchars((string)$this->_['instanceId'], ENT_QUOTES); ?>-returntokenbudget"><?php echo $mbTextEsc('return_token_budget', 'Return token budget'); ?></label>
							<select id="<?php echo htmlspecialchars((string)$this->_['instanceId'], ENT_QUOTES); ?>-returntokenbudget" name="returnTokenBudget">
								<option value=""><?php echo $mbTextEsc('default', 'default'); ?></option>
								<option value="default"><?php echo $mbTextEsc('default', 'default'); ?></option>
								<option value="unlimited"><?php echo $mbTextEsc('unlimited', 'unlimited'); ?></option>
							</select>
						</div>

						<div>
							<label for="<?php echo htmlspecialchars((string)$this->_['instanceId'], ENT_QUOTES); ?>-maxresults"><?php echo $mbTextEsc('max_results', 'Max results'); ?></label>
					...
					<div class="searchcfg-field searchcfg-field-checkbox">
						<label class="searchcfg-checkbox">
							...
							<span><?php echo $mbTextEsc('external_web_access', 'External web access'); ?></span>
						</label>
					</div>

					<div class="searchcfg-field">
						<label for="<?php echo htmlspecialchars((string)$this->_['instanceId'], ENT_QUOTES); ?>-alloweddomains"><?php echo $mbTextEsc('allowed_domains', 'Allowed domains'); ?></label>
					...
					<div class="searchcfg-field">
						<label for="<?php echo htmlspecialchars((string)$this->_['instanceId'], ENT_QUOTES); ?>-blockeddomains"><?php echo $mbTextEsc('blocked_domains', 'Blocked domains'); ?></label>
				...
				<div data-role="testresult" class="searchcfg-test-result" style="display:none">
					<div data-role="testmeta" class="searchcfg-test-meta"></div>
					<pre data-role="testpreview" class="searchcfg-test-preview"></pre>
				</div>

				<div class="searchcfg-actions">
					<button type="submit" class="primary"><?php echo $mbTextEsc('save_search_service', 'Save search service'); ?></button><button type="button" data-role="test"><?php echo $mbTextEsc('test_search_service', 'Test search service'); ?></button>
					<button type="button" data-role="delete" disabled><?php echo $mbTextEsc('delete_search_service', 'Delete search service'); ?></button>
				</div>
			</form>
		</div>
	</div>
...
.searchcfg-test-result{margin-top:12px;border:1px solid #b9d3b9;background:#f8fff8;border-radius:6px;padding:10px 12px}.searchcfg-test-result.failed{border-color:#d88;background:#fff5f5}.searchcfg-test-meta{font-size:12px;color:#466846;margin-bottom:8px}.searchcfg-test-result.failed .searchcfg-test-meta{color:#8a3a3a}.searchcfg-test-preview{margin:0;max-height:240px;overflow:auto;white-space:pre-wrap;word-break:break-word;border:1px solid #d9e6d9;background:#fff;padding:10px;border-radius:4px;font-family:Consolas,monospace;font-size:12px;color:#333}
</style>
	...
	function init() {
		...
		function updateModelState() {
			...
			if (refs.modelrequiredlabel) {
				refs.modelrequiredlabel.textContent = required ? "(required)" : mbText('optional', '(optional)');
			}
		...
		function setEditMode(editing) {
			...
			if (editing) {
				refs.legend.textContent = mbText('edit_search_service', 'Edit search service');
				refs.idhint.textContent = mbText('technical_search_service_id_is_fixed_for_existing_entries_create_a_new_entry_if_you_need_another_key', 'Technical search service id is fixed for existing entries. Create a new entry if you need another key.');
			} else {
				refs.legend.textContent = mbText('create_search_service', 'Create search service');
				refs.idhint.textContent = mbText('technical_search_service_id_agent_resources_use_this_id_to_resolve_the_configured_search_service', 'Technical search service id. Agent resources use this id to resolve the configured search service.');
			}
		}
		...
		function renderDriverOptions(selected) {
			...
			if (drivers.length === 0) {
				const option = document.createElement("option");
				option.value = "";
				option.textContent = mbText('no_search_drivers_available', 'No search drivers available');
		...
		async function saveCurrent() {
			...
			if (!id) {
				showFeedback(mbText('search_service_id_is_required', 'Search service id is required.'), "error");
				return;
			}
			...
			if (!model && isModelRequiredForDriver(driver)) {
				showFeedback(mbText('model_is_required_for_the_selected_driver', 'Model is required for the selected driver.'), "error");
				return;
			}
			...
			showFeedback(mbText('search_service_saved', 'Search service saved.'), "success");
		...
		async function removeCurrent() {
			...
			if (!id) {
				showFeedback(mbText('no_search_service_selected', 'No search service selected.'), "error");
				return;
			}

			if (!window.confirm(mbText('delete_search_service_confirm', "Delete search service '{id}'?", {id}))) {
				return;
			}
			...
			showFeedback(mbText('search_service_deleted', 'Search service deleted.'), "success");
