...
(function() {
	...
	function init() {
		...
		function updateAuthUi() {
			const authType = normalizeKey(refs.authType.value);
			const secretMode = normalizeKey(refs.secretMode.value);

			if (authType === "none") {
				refs.secretMode.value = "fixed";
				refs.secretMode.disabled = true;
				refs.secretValue.disabled = true;
				refs.authHeaderName.disabled = true;
				refs.secretValue.value = "";
				refs.authHeaderName.value = "";
				refs.secretvaluelabel.textContent = "Secret value";
				refs.secrethint.textContent = "No secret is used when auth type is none.";
				return;
			}

			refs.secretMode.disabled = false;
			refs.secretValue.disabled = false;
			refs.authHeaderName.disabled = false;

			if (authType === "bearer" && !refs.authHeaderName.value) refs.authHeaderName.value = "Authorization";

			if (secretMode === "env") {
				refs.secretvaluelabel.textContent = "Environment variable";
				refs.secretValue.placeholder = "OPENAI_API_KEY";
				refs.secrethint.textContent = "Enter the environment variable name. The runtime resolves it when the connection is used.";
				return;
			}

			refs.secretvaluelabel.textContent = "Secret value";
			refs.secretValue.placeholder = "sk-...";
			refs.secrethint.textContent = "Enter the API key directly. The value will be stored in the SettingsStore.";
		}
		...
		function applyDriverDefaults(force) {
			...
			const auth = defaults.auth && typeof defaults.auth === "object" ? defaults.auth : {};
			...
			if (force) {
				...
				refs.secretMode.value = auth.secretMode || "fixed";
		...
		function resetForm() {
			refs.form.reset();
			refs.id.value = refs.name.value = refs.baseUrl.value = refs.authHeaderName.value = refs.secretValue.value = "";
		...
		function fillForm(connection) {
			...
			refs.secretMode.value = connection.secretMode || "fixed";
			refs.secretValue.value = connection.secretValue || "";
		...
		function renderRows() {
			...
			state.connections.forEach(connection => {
				...
				const driverText = connection.driverLabel || connection.driver || "";
				const headerText = connection.authHeaderName || "";
				...
				tr.innerHTML =
					...
					"<td class='secret-col' title='" + esc(connection.secretMode || "fixed") + "'><span>" + esc(connection.secretMode || "fixed") + "</span><br><span class='ccad-subline'>" + esc(maskSecret(connection.secretValue, connection.secretMode)) + "</span></td>" +
		...
		async function loadList(preselectId) {
			...
			const selected = findConnection(preselectId || state.selectedId);
			if (selected) fillForm(selected);
			else if (!state.connections.length) resetForm();
		}
		...
		function readOptionsJson() {
			...
			try {
				...
				if (!parsed || typeof parsed !== "object" || Array.isArray(parsed)) {
					printOutput("Options must be a JSON object.", "error");
					return null;
				}

				return JSON.stringify(parsed);
			} catch (e) {
				printOutput("Options must be valid JSON:\n" + e.message, "error");
				return null;
			}
		}
		...
		async function saveCurrent() {
			...
			const type = normalizeKey(refs.type.value);
			const baseUrl = String(refs.baseUrl.value || "").trim();
			const authType = normalizeKey(refs.authType.value);
			const authHeaderName = String(refs.authHeaderName.value || "").trim();
			const secretMode = normalizeKey(refs.secretMode.value || "fixed");
			const secretValue = String(refs.secretValue.value || "").trim();
			const timeoutSeconds = String(refs.timeoutSeconds.value || "").trim();
			const scope = normalizeKey(refs.scope.value || "global");
			...
			if (!id || !name || !driver || !type) {
				printOutput("Connection id, name, driver and type are required.", "error");
				return;
			}

			if (type === "http" && !baseUrl) {
				printOutput("Base URL is required for HTTP connections.", "error");
				return;
			}

			if (authType !== "none" && !secretValue) {
				printOutput("Secret value is required unless auth type is none.", "error");
				return;
			}

			const json = await callApi({action:"save", id, name, driver, type, baseUrl, authType, authHeaderName, secretMode, secretValue, timeoutSeconds, scope, enabled: refs.enabled.checked ? "1" : "0", options});
			if (!json) return;

			const connection = json.data && json.data.connection ? json.data.connection : null;
			printOutput({status:"saved", group:configGroup, connection: connection ? Object.assign({}, connection, {secretValue: maskSecret(connection.secretValue, connection.secretMode)}) : null}, "success");
			await loadList(connection && connection.id ? connection.id : id);
		}
		...
		async function removeCurrent() {
			...
			if (!id) {
				printOutput("No connection selected.", "error");
				return;
			}
		...
		refs.driver.addEventListener("change", () => applyDriverDefaults(false));
		refs.authType.addEventListener("change", updateAuthUi);
		refs.secretMode.addEventListener("change", updateAuthUi);
