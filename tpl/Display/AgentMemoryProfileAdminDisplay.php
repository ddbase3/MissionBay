...
<div class="agent-preset-profile-admin-shell" data-agent-preset-profile-admin data-profile-kind="<?php echo $e($profileKind); ?>">
	...
	<div class="agent-preset-profile-admin-panel">
		<label><?php echo $mbTextEsc('search', 'Search'); ?> <input type="search" class="agent-preset-profile-admin-input" data-search placeholder="<?php echo $mbTextEsc('id_label_or_preset', 'ID, label or preset'); ?>"></label>
		<button type="button" class="agent-preset-profile-admin-button" data-clear><?php echo $mbTextEsc('clear', 'Clear'); ?></button>
		<span class="agent-preset-profile-admin-spacer"></span>
		<a class="agent-preset-profile-admin-button" href="<?php echo $e($componentPresetAdminUrl); ?>"><?php echo $mbTextEsc('component_presets', 'Component Presets'); ?></a>
		<button type="button" class="agent-preset-profile-admin-button" data-reload><?php echo $mbTextEsc('reload', 'Reload'); ?></button>
		<button type="button" class="agent-preset-profile-admin-button agent-preset-profile-admin-button-primary" data-new><?php echo $mbTextEsc('new_profile', 'New profile'); ?></button>
	</div>
	<div class="agent-preset-profile-admin-panel agent-preset-profile-admin-panel--filters">
		<label><?php echo $mbTextEsc('status', 'Status'); ?> <select class="agent-preset-profile-admin-select" data-enabled-filter><option value=""><?php echo $mbTextEsc('all_states', 'All states'); ?></option><option value="1"><?php echo $mbTextEsc('enabled', 'Enabled'); ?></option><option value="0"><?php echo $mbTextEsc('disabled', 'Disabled'); ?></option></select></label>
	</div>
	<div class="agent-preset-profile-admin-main">
		<div class="agent-preset-profile-admin-table-scroll">
			<table class="agent-preset-profile-admin-table">
				<thead><tr><th style="width:25%"><?php echo $mbTextEsc('profile', 'Profile'); ?></th><th style="width:12%"><?php echo $mbTextEsc('status', 'Status'); ?></th><th style="width:32%"><?php echo $mbTextEsc('configured_presets', 'Configured presets'); ?></th><th><?php echo $mbTextEsc('description', 'Description'); ?></th><th style="width:150px"><?php echo $mbTextEsc('actions', 'Actions'); ?></th></tr></thead>
				<tbody data-table-body><tr><td colspan="5" class="agent-preset-profile-admin-empty"><?php echo $mbTextEsc('loading_profiles', 'Loading profiles...'); ?></td></tr></tbody>
			</table>
		</div>
	</div>
	<div class="agent-preset-profile-admin-status" data-status><strong><?php echo $mbTextEsc('last_action', 'Last action:'); ?></strong> <?php echo $mbTextEsc('waiting_for_initialization', 'Waiting for initialization.'); ?></div>
</div>
...
<div class="agent-preset-profile-admin-modal" data-modal data-profile-kind="<?php echo $e($profileKind); ?>" hidden>
	<div class="agent-preset-profile-admin-dialog" role="dialog" aria-modal="true">
		<div class="agent-preset-profile-admin-dialog-header">
			...
			<span class="agent-preset-profile-admin-spacer"></span>
			<button type="button" class="agent-preset-profile-admin-button" data-close><?php echo $mbTextEsc('close', 'Close'); ?></button>
		</div>
		<div class="agent-preset-profile-admin-dialog-body">
			<form data-form>
				...
				<div class="agent-preset-profile-admin-form">
					<div><label class="agent-preset-profile-admin-label"><?php echo $mbTextEsc('technical_id', 'Technical ID'); ?></label><input class="agent-preset-profile-admin-input" name="id" required pattern="[a-z0-9._-]+"></div>
					<div><label class="agent-preset-profile-admin-label"><?php echo $mbTextEsc('label', 'Label'); ?></label><input class="agent-preset-profile-admin-input" name="label"></div>
					<div class="agent-preset-profile-admin-field-full"><label><input type="checkbox" name="enabled" checked> <?php echo $mbTextEsc('enabled_lowercase', 'enabled'); ?></label></div>
					<div class="agent-preset-profile-admin-field-full"><label class="agent-preset-profile-admin-label"><?php echo $mbTextEsc('description', 'Description'); ?></label><textarea class="agent-preset-profile-admin-textarea" name="description"></textarea></div>
					<div class="agent-preset-profile-admin-field-full">
						...
						<p class="agent-preset-profile-admin-help"><?php echo $mbTextEsc('only_already_configured_component_presets_are_listed_their_saved_configuration_is_used_unchanged_at_runtime', 'Only already configured Component Presets are listed. Their saved configuration is used unchanged at runtime.'); ?></p>
					</div>
				</div>
			</form>
		</div>
		<div class="agent-preset-profile-admin-dialog-footer">
			<button type="button" class="agent-preset-profile-admin-button agent-preset-profile-admin-button-danger" data-delete hidden><?php echo $mbTextEsc('delete', 'Delete'); ?></button>
			<span class="agent-preset-profile-admin-spacer"></span>
			<button type="button" class="agent-preset-profile-admin-button" data-cancel><?php echo $mbTextEsc('cancel', 'Cancel'); ?></button>
			<button type="button" class="agent-preset-profile-admin-button agent-preset-profile-admin-button-primary" data-save><?php echo $mbTextEsc('save', 'Save'); ?></button>
		</div>
	</div>
</div>
...
(function(){
	...
	function setStatus(message,error){status.dataset.error=error?'1':'0';status.innerHTML='';const strong=document.createElement('strong');strong.textContent=error?mbText('error_prefix','Error: '):mbText('last_action','Last action: ');status.appendChild(strong);status.appendChild(document.createTextNode(String(message||'')));}
	...
	function renderRows(rows){body.innerHTML='';if(!Array.isArray(rows)||rows.length===0){const tr=document.createElement('tr');const td=document.createElement('td');td.colSpan=5;td.className='agent-preset-profile-admin-empty';td.textContent=mbText('no_profiles_match_the_current_filter', 'No profiles match the current filter.');tr.appendChild(td);body.appendChild(tr);return;}rows.forEach(function(row){const tr=document.createElement('tr');let td=document.createElement('td');td.appendChild(cell(row.label||row.profile_id,row.profile_id));tr.appendChild(td);td=document.createElement('td');const pills=document.createElement('div');pills.className='agent-preset-profile-admin-pill-row';pills.appendChild(pill(row.enabled_label,row.enabled?'agent-preset-profile-admin-pill-enabled':'agent-preset-profile-admin-pill-disabled'));td.appendChild(pills);tr.appendChild(td);td=document.createElement('td');td.appendChild(cell(String(row.preset_count||0)+' preset'+(Number(row.preset_count||0)===1?'':'s'),row.preset_text||'No presets'));tr.appendChild(td);td=document.createElement('td');td.textContent=row.description||'';tr.appendChild(td);td=document.createElement('td');const actions=document.createElement('div');actions.className='agent-preset-profile-admin-actions';actions.appendChild(button('Edit',function(){openExisting(row.profile_id);}));actions.appendChild(button(mbText('copy', 'Copy'),function(){openCopy(row.profile_id);}));td.appendChild(actions);tr.appendChild(td);body.appendChild(tr);});}
	async function loadPage(){try{const json=await post({mode:'page',page:1,pageSize:250,search:search.value.trim(),filters:{enabled:enabledFilter.value},sort:[{key:'profile_id',dir:'asc'}]});renderRows(json.data||[]);setStatus(mbText('loaded_profiles_from_group', 'Loaded {count} profile(s) from {group}.', {count:String(json.total||0),group:SETTINGS_GROUP}));}catch(error){body.innerHTML='';const row=document.createElement('tr');const cell=document.createElement('td');cell.colSpan=5;cell.className='agent-preset-profile-admin-empty';cell.textContent=mbText('unable_to_load_profiles','Unable to load profiles.');row.appendChild(cell);body.appendChild(row);setStatus(error.message,true);}}
	...
	async function save(){const payload={mode:'save',old_id:form.elements.old_id.value,id:form.elements.id.value,label:form.elements.label.value,description:form.elements.description.value,enabled:form.elements.enabled.checked,presets:collectPresets()};try{const json=await post(payload);closeModal();setStatus(mbText('profile_prefix', 'Profile ')+json.action+'.');await loadPage();}catch(error){setStatus(error.message,true);}}
	async function remove(){const id=form.elements.old_id.value;if(!id||!window.confirm('Delete profile '+id+'?'))return;try{await post({mode:'delete',id:id});closeModal();setStatus(mbText('profile_deleted', 'Profile deleted.'));await loadPage();}catch(error){setStatus(error.message,true);}}
