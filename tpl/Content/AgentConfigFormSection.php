...
<div id="<?php echo $e($rootId); ?>" class="base3-agent-config-root" data-base3-agent-runtime-config-root="missionbay">
	<div class="base3-agent-config-section">
		...
		<div class="base3-agent-config-row">
			<label class="base3-agent-config-label" for="<?php echo $e($formId); ?>_llm"><?php echo $e($t('llm_label', 'LLM')); ?></label>
			<div><select id="<?php echo $e($formId); ?>_llm" name="llm"><option value=""><?php echo $e($t('use_agent_flow_value', 'Use AgentFlow value')); ?></option>
<?php foreach($llmOptions as $llm): $id=(string)($llm['id']??''); if($id==='') continue; $label=(string)($llm['label']??$id); ?>
				<option value="<?php echo $e($id); ?>"<?php echo $selected($values['llm']??'', $id); ?>><?php echo $e($label . (!empty($llm['model']) ? ' / '.$llm['model'] : '') . (empty($llm['enabled']) ? ' [' . $t('disabled_marker', 'disabled') . ']' : '')); ?></option>
...
<?php endforeach; ?>
	...
	<details class="base3-agent-config-expert"<?php echo !empty($values['expert_overrides_enabled']) ? ' open="open"' : ''; ?>>
		...
		<div class="base3-agent-config-expert-body">
			<div class="base3-agent-config-section">
				<h3><?php echo $e($t('agent_flow_section', 'AgentFlow')); ?></h3>
				<div class="base3-agent-config-row">
					<label class="base3-agent-config-label" for="<?php echo $e($formId); ?>_agent_flow"><?php echo $e($t('flow_json_label', 'Flow JSON')); ?></label>
					<div>
						...
						<p class="base3-agent-config-help"><?php echo $e($t('flow_help', 'Required MissionBay flow definition. It must contain at least one node.')); ?></p>
					</div>
				</div>
			</div>
...
<?php endforeach; ?>
			...
			<div class="base3-agent-config-export">
				...
				<span class="base3-agent-config-export-status" data-agent-config-export-status><?php echo $e($t('export_help', 'Exports current form values plus resolved LLM, orchestrator, tool, memory, context and component-preset configuration. Secret-like fields are redacted.')); ?></span>
			</div>
		</div>
	</details>
</div>
...
(function(){
	...
	function addComponent(component){component=component||{}; var row=document.createElement('div'); row.className='base3-agent-config-component-row'; row.dataset.componentRow='1';
		...
		var remove=document.createElement('button'); remove.type='button'; remove.textContent=i18n.remove; remove.onclick=function(){row.remove();sync()}; row.appendChild(remove); items.appendChild(row); row.addEventListener('change',sync); row.addEventListener('input',sync); nextIndex++;
	}
