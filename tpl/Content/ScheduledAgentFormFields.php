<?php
	...
	$formId = (string)($this->_['form_id'] ?? 'base3_scheduled_agent_config');
	$group = 'scheduled-agent';
	$name = (string)($this->_['name'] ?? 'default');
	$rootId = $formId . '_scheduled_agent_fields';
...
<style>
	.base3-scheduled-agent-fields,
	.base3-scheduled-agent-fields * {
		box-sizing: border-box;
	}

	.base3-scheduled-agent-section {
		margin: 0 0 20px;
		padding: 16px;
	...
	.base3-scheduled-agent-section h3 {
		margin: 0 0 14px;
		font-size: 18px;
	}

	.base3-scheduled-agent-row {
		display: grid;
	...
	.base3-scheduled-agent-row:last-child {
		margin-bottom: 0;
	}

	.base3-scheduled-agent-label {
		padding-top: 7px;
		font-weight: bold;
	}

	.base3-scheduled-agent-fields input[type="text"],
	.base3-scheduled-agent-fields input[type="number"],
	.base3-scheduled-agent-fields select,
	.base3-scheduled-agent-fields textarea {
		width: 100%;
		max-width: 620px;
		min-height: 34px;
		padding: 6px 8px;
	...
	.base3-scheduled-agent-user-prompt {
		min-height: 150px;
	}

	.base3-scheduled-agent-json {
		min-height: 110px;
	}

	.base3-scheduled-agent-help {
		max-width: 760px;
		margin: 5px 0 0;
		color: #666;
		font-size: 12px;
	...
	.base3-scheduled-agent-checkbox-row {
		display: inline-flex;
		align-items: center;
		gap: 7px;
		min-height: 34px;
	...
	.base3-scheduled-agent-policy-fields {
		display: grid;
		gap: 10px;
		max-width: 760px;
		margin-top: 10px;
	...
	.base3-scheduled-agent-policy-field {
		display: grid;
	...
	.base3-scheduled-agent-policy-field-required .base3-scheduled-agent-policy-label::after {
		content: " *";
		color: #8a1f1f;
	}

	.base3-scheduled-agent-policy-label {
		padding-top: 7px;
		font-weight: bold;
		font-size: 12px;
	...
	.base3-scheduled-agent-policy-control {
		min-width: 0;
	}

	.base3-scheduled-agent-policy-empty {
		padding: 8px;
		border: 1px solid #e2e2e2;
		border-radius: 4px;
		background: #fafafa;
...
<div
	...
	class="base3-scheduled-agent-fields"
	data-base3-scheduled-agent-fields="1"
>
	<input type="hidden" name="scheduled_agent_config_action" value="save" />
	<input type="hidden" name="scheduled_agent_config_group" value="<?php echo $e($group); ?>" />
	<input type="hidden" name="scheduled_agent_config_name" value="<?php echo $e($name); ?>" />

	<div class="base3-scheduled-agent-section">
		<h3>Scheduled agent</h3>

		<div class="base3-scheduled-agent-row">
			<div class="base3-scheduled-agent-label">Active</div>
			<div>
				<label class="base3-scheduled-agent-checkbox-row">
		...
		<div class="base3-scheduled-agent-row">
			<label for="<?php echo $e($formId); ?>_label" class="base3-scheduled-agent-label">Label</label>
			<div>
				<input id="<?php echo $e($formId); ?>_label" type="text" name="label" class="form-control" value="<?php echo $e($values['label'] ?? ''); ?>" />
				<p class="base3-scheduled-agent-help">
		...
		<div class="base3-scheduled-agent-row">
			<label for="<?php echo $e($formId); ?>_user_prompt" class="base3-scheduled-agent-label">User prompt</label>
			<div>
				<textarea id="<?php echo $e($formId); ?>_user_prompt" name="user_prompt" class="form-control base3-scheduled-agent-user-prompt"><?php echo $e($values['user_prompt'] ?? ''); ?></textarea>
				<p class="base3-scheduled-agent-help">
					Prompt sent as the user message when the scheduled job executes this agent.
				</p>
			</div>
		</div>
	</div>
	...
	<div class="base3-scheduled-agent-section">
		...
		<div class="base3-scheduled-agent-row">
			<label for="<?php echo $e($formId); ?>_policy" class="base3-scheduled-agent-label">Policy</label>
			<div>
				<select id="<?php echo $e($formId); ?>_policy" name="policy" class="form-control" data-base3-scheduled-agent-policy-select="1">
...
<?php } ?>
				...
				<input type="hidden" name="policy_data_json" data-base3-scheduled-agent-policy-data-json="1" value="{}" />
				<input type="hidden" name="policy_data_b64" data-base3-scheduled-agent-policy-data-b64="1" value="" />
				<div class="base3-scheduled-agent-policy-fields" data-base3-scheduled-agent-policy-fields="1"></div>
			</div>
		</div>
	</div>
...
(function() {
	...
	if (!root || root.getAttribute('data-base3-scheduled-agent-ready') === '1') {
		return;
	}

	root.setAttribute('data-base3-scheduled-agent-ready', '1');
	...
	var policySelect = root.querySelector('[data-base3-scheduled-agent-policy-select]');
	var policyFields = root.querySelector('[data-base3-scheduled-agent-policy-fields]');
	var policyDataJson = root.querySelector('[data-base3-scheduled-agent-policy-data-json]');
	var policyDataB64 = root.querySelector('[data-base3-scheduled-agent-policy-data-b64]');
	...
	function readPolicyControlValue(control) {
		var type = control.getAttribute('data-base3-scheduled-agent-policy-type') || 'string';
	...
	function buildPolicyDataFromControls() {
		var data = {};
		var controls = root.querySelectorAll('[data-base3-scheduled-agent-policy-key]');

		controls.forEach(function(control) {
			var key = control.getAttribute('data-base3-scheduled-agent-policy-key') || '';
	...
	function renderPolicyControl(key, property, value) {
		...
		if (type === 'boolean') {
			...
			wrapper.className = 'base3-scheduled-agent-checkbox-row';
			hidden.type = 'hidden';
	...
	function renderPolicyFields(policyId, data) {
		...
		if (keys.length === 0) {
			policyFields.appendChild(createElement('base3-scheduled-agent-policy-empty', 'This policy does not expose configurable fields.'));
			syncPolicyDataJson();
			return;
		}
		...
		keys.forEach(function(key) {
			...
			var row = createElement('base3-scheduled-agent-policy-field' + (required.indexOf(key) !== -1 ? ' base3-scheduled-agent-policy-field-required' : ''));
			var label = createElement('base3-scheduled-agent-policy-label', key);
			var controlCell = createElement('base3-scheduled-agent-policy-control');
	...
	root.__base3ScheduledAgentRenderPolicyFields = function(policyId, data) {
		renderPolicyFields(policyId, data || {});
	};

	root.__base3ScheduledAgentUpdateValues = function(values) {
		...
		var group = root.querySelector('[name="scheduled_agent_config_group"]');
		var name = root.querySelector('[name="scheduled_agent_config_name"]');
		...
		if (group && values.scheduled_agent_config_group) {
			group.value = values.scheduled_agent_config_group;
		}

		if (name && values.scheduled_agent_config_name) {
			name.value = values.scheduled_agent_config_name;
		}
		...
		root.__base3ScheduledAgentPrepareSubmit();
	};

	root.__base3ScheduledAgentPrepareSubmit = function() {
		syncPolicyDataJson();
