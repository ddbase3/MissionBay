...
class ScheduledAgentConfigDisplay implements IDisplay {

	protected const FORM_ACTION_SAVE = 'save';
	protected const DEFAULT_GROUP = 'scheduled-agent';
	...
	public static function getName(): string {
		return 'scheduledagentconfigdisplay';
	}
	...
	public function getOutput(string $out = 'html', bool $final = false): string {
		...
		$this->view->setTemplate('Content/ScheduledAgentConfigDisplay.php');
	...
	public function getHelp(): string {
		return 'Configure one scheduled MissionBay agent instance and store its settings through ISettingsStore.';
	}
	...
	protected function getJsonOutput(bool $final): string {
		...
		if ($action === '') {
			$action = trim((string)$this->request->request('scheduled_agent_config_action', ''));
		}
	...
	protected function getContext(bool $allowRequestContext): array {
		...
		if ($allowRequestContext && $name === '') {
			$name = trim((string)$this->request->request('scheduled_agent_config_name', ''));
		}
		...
		$title = trim((string)($this->data['title'] ?? 'Scheduled Agent Configuration'));
		if ($title === '') {
			$title = 'Scheduled Agent Configuration';
		}

		$description = trim((string)($this->data['description'] ?? 'Configure the selected scheduled agent instance.'));
		...
		return [
			...
			'form_id' => 'base3_scheduled_agent_config_' . md5($group . '/' . $name),
	...
	protected function isSaveRequest(array $context): bool {
		if ((string)$this->request->request('scheduled_agent_config_action') !== self::FORM_ACTION_SAVE) {
			return false;
		}

		$postedGroup = trim((string)$this->request->request('scheduled_agent_config_group'));
		$postedName = trim((string)$this->request->request('scheduled_agent_config_name'));
