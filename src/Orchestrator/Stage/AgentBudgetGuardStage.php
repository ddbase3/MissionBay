...
/**
 * AgentBudgetGuardStage
 *
 * Performs deterministic budget preflight checks at configured execution
 * boundaries. The same implementation is mounted before model decisions and
 * before tool execution. It evaluates only exact runtime counters and
 ...
 */
final class AgentBudgetGuardStage implements IAgentStage {
	...
	public function __construct(
		private readonly string $id = 'budget-guard',
		private readonly string $stageName = 'budget-guard',
		private readonly string $checkpoint = self::CHECKPOINT_MODEL
	) {
		if (!in_array($this->checkpoint, [self::CHECKPOINT_MODEL, self::CHECKPOINT_TOOLS, self::CHECKPOINT_FINAL], true)) {
			throw new \InvalidArgumentException('Unsupported agent budget checkpoint: ' . $this->checkpoint);
		}
	}

	public static function getName(): string {
		return 'agentbudgetguardstage';
	}
	...
	public function getDescription(): string {
		if ($this->checkpoint === self::CHECKPOINT_TOOLS) {
			return 'Checks the updated AI usage and projected tool-call count after action policy and before tool execution.';
		}

		if ($this->checkpoint === self::CHECKPOINT_FINAL) {
			return 'Checks exact elapsed time, AI-operation count, and provider-reported usage before the final response generation starts.';
		}

		return 'Checks exact elapsed time, AI-operation count, and provider-reported usage before another model decision starts.';
	}
	...
	public function supports(IAgentContext $context): bool {
		return $context->getVar(AgentToolLoopContextKeys::PHASE) === $this->getExpectedPhase()
			&& ($this->checkpoint === self::CHECKPOINT_FINAL || $context->getVar(AgentToolLoopContextKeys::COMPLETED) !== true)
	...
	public function process(IAgentContext $context): AgentStageResult {
		...
		$pendingToolCallCount = $this->checkpoint === self::CHECKPOINT_TOOLS
		...
		$exceededLimits = $this->findExceededLimits(
			...
			$this->checkpoint === self::CHECKPOINT_TOOLS ? $projectedToolCallCount : null,
			$elapsedMs
		);
	...
	private function getExpectedPhase(): string {
		return match($this->checkpoint) {
