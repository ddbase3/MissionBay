 ...
 *
 * Adapter for DeepSeek's OpenAI-compatible Chat Completion API.
 * Supports:
 * - chat()
 * - raw()
 * - stream()
...
class DeepSeekChatModelAgentResource extends AbstractAgentResource implements IAiChatModel {
	...
	public static function getName(): string {
		return 'deepseekchatmodelagentresource';
	}

	public function getDescription(): string {
		return 'Connects to DeepSeek OpenAI-compatible Chat Completion API.';
	}
	...
	public function raw(array $messages, array $tools = []): mixed {
		...
		if (empty($opts['apikey'])) {
			throw new \RuntimeException("Missing API key for DeepSeek model.");
		}
		...
		if (curl_errno($ch)) {
			throw new \RuntimeException('DeepSeek request failed: ' . curl_error($ch));
		}
		...
		if ($http !== 200) {
			throw new \RuntimeException("DeepSeek error HTTP $http: $result");
		}

		$data = json_decode($result, true);
		if (!is_array($data)) {
			throw new \RuntimeException("Invalid JSON from DeepSeek: " . substr($result ?? '', 0, 200));
		}
