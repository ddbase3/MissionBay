 ...
 *
 * Adapter for xAI (Grok) OpenAI-compatible Chat Completion API.
 * Supports:
 * - chat()
 * - raw()
 * - stream()
...
class GrokChatModelAgentResource extends AbstractAgentResource implements IAiChatModel {
	...
	public function getDescription(): string {
		return 'Connects to xAI Grok OpenAI-compatible Chat Completion API.';
	}
	...
	public function raw(array $messages, array $tools = []): mixed {
		...
		if (empty($opts['apikey'])) {
			throw new \RuntimeException("Missing API key for Grok model.");
		}
		...
		if (curl_errno($ch)) {
			throw new \RuntimeException('Grok request failed: ' . curl_error($ch));
		}
		...
		if ($http !== 200) {
			throw new \RuntimeException("Grok error HTTP $http: $result");
		}
